...
现在，你已准备好重新创建自定义命名实体识别项目。 此项目提供了一个用于生成、训练和部署模型的工作场所。
...
1. **我们“下一个文档”按钮移动到下一个文档**，并继续将文本分配给整个文档集的相应实体，并将其全部添加到训练数据集。
...
1. 更新配置值，以包含创建的 Azure 语言资源的**终结点**和**密钥**（位于 Azure 门户中 Azure AI 语言资源的“密钥和终结点”**** 页面上）。 fil 应已包含自定义实体提取模型的项目和部署名称。
1. 保存此配置文件。
